...
async def delete_job(job_id: str):
    job = await JobModel.find_one(JobModel.job_id == job_id)
    if not job:
...
async def pause_job(job_id: str):
    ...
    await JobModel.find_one(JobModel.job_id == job_id).update({"$set": {"status": "paused"}}) # await JobModel.find_one(JobModel.id == job_id).update({"$set": {"status": "paused"}})
    return {"message": "Job paused"}

@router.post("/{job_id}/resume")
async def resume_job(job_id: str):
    job = await JobModel.find_one(JobModel.job_id == job_id) # job = await JobModel.get(job_id)
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    await add_job(job)
